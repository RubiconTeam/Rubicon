...
public partial class CanvasItemSpace : Node2D
{
    ...
    {
        // Init stage
        Stage = GD.Load<Stage2D>($"res://Resources/Stages/{meta.Stage}.tscn"); // TODO: Check if stage even exists
        AddChild(Stage);
    ...
    public void AddCharacter(CharacterMeta meta)
    {
        ...
        if (!ResourceLoader.Exists(path))
        {
            ...
            character = ResourceLoader.Load<Character2D>(fallBack);
        }
        else
        {
            character = ResourceLoader.Load<Character2D>(path);
        }
